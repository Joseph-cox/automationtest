#PBS-1228
Feature: UVL Search Results - Certified by Volvo & 'Read more' content page
...
Then there should be a 'Certified by Volvo' button present

Scenario: To verify the button's functionality
When the Results page loads
Then if any of the Volvo 'Certified' icon is selected
Then an overlay will pop-up on the screen

Scenario: To verify that the overlay can be closed
When the 'Certified by Volvo' overlay it is opened
...
Then if scrolling down the page
Then the benefits could be viewed and read
Then similarly, if the user returns back to the SRP
Then the scrolling on the page is still functional
