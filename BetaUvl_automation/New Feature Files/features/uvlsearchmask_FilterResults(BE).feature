...
Feature: UVL Search Results - Filter Results
Background:
...
Scenario: To verify the Warranty filter works correctly
When the filter is not toggled to active
Then the whole available stock will be displayed into the results
Then when toggling the filter to be active
Then only vehicles with warranties will be returned into the results