...
Background
As a Modix UVL user I would like to be offered different purchasing options that could fit my budget
...
And by selecting the 'Back' button
And the fields present here are viewed
And the pre-selected values will still be saved and stored
...
When opening the finance iFrame for any of the vehicles within the model year ranges
...
Scenario: To verify if the dealership can be contacted via the iFrame
When opening the finance iFrame
Then proceeding further to the quote
Then selecting the Contact CTA
And the user is able to submit the form

Scenario: To verify if the iFrame can be closed
When the finance iFrame is being opened
Then the user selects the 'X' or 'Close' button
Then the iFrame should close
