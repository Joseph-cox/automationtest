...
As a Modix UVL user
I want to be able to select the preferred language and see the UVL updated based on the my selection
...
Then after checking the header & footer
 And the page items will be updated to the chosen language
...
And  the vehicle data updates accordingly
...
Scenario: To verify if the Standard & Optional Equipment are interchangeable and opening the details page of any vehicle
When switching the preferred language
Then scroll down to the Equipment section to verify the options present here
And if available, their description should reflect the latest language selection made

Scenario: To verify the DLP banners
Given that the user navigates to the DLP of any centre and after checking the banner, this should be in the language selected
